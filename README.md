...
[![PyPI](https://img.shields.io/pypi/v/codingagent)](https://pypi.org/project/codingagent/)
[![Python](https://img.shields.io/badge/python-3.10+-blue)](https://python.org)
[![License: MIT](https://img.shields.io/badge/license-MIT-green)](LICENSE)
[![Tests](https://github.com/he-yufeng/CodingAgent/actions/workflows/ci.yml/badge.svg)](https://github.com/he-yufeng/CodingAgent/actions)
...
CodingAgent is not another AI coding tool. It's a **blueprint** — the [nanoGPT](https://github.com/karpathy/nanoGPT) of coding agents. Read it, fork it, build your own.

---

```
$ codingagent -m kimi-k2.5
...
| Pattern | Claude Code | CodingAgent |
|---|---|---|
...
```bash
git clone https://github.com/he-yufeng/CodingAgent.git
cd CodingAgent
pip install -e .
```

The installed command is lowercase: `codingagent`. On Linux and macOS, `CodingAgent` is a different name and will not resolve.
...
codingagent -m anthropic/claude-opus-4-6
...
export CODINGAGENT_PROVIDER=litellm
export CODINGAGENT_MODEL=anthropic/claude-3-haiku   # any LiteLLM model string
...
```python
from codingagent import Agent, LLM
...
```python
from codingagent.tools.base import Tool
...
Saved session IDs are sanitized before they become filenames, so resume data stays inside `~/.codingagent/sessions`.

## How It Compares

|  | Claude Code | Claw-Code | Aider | CodingAgent |
|---|---|---|---|---|
...
I wrote [7 articles](article/) breaking down Claude Code's architecture — the agent loop, tool system, context compression, streaming executor, multi-agent, and 44 hidden feature flags. If you want to understand *why* CodingAgent is designed this way, start there.

## FAQ

**Does CodingAgent support Skills / Subagents / MCP?**

No, and that's intentional. CodingAgent is the minimal runnable core — agent loop, tools, streaming, compaction. Skills, Subagents, MCP, hooks, and plugins are upper-layer features that Claude Code layers on top; if CodingAgent had them too it would stop being a teaching artifact. The architecture articles above cover how those systems work in Claude Code, so you can add them yourself if you need to.
...
- **[AnyCoder](https://github.com/he-yufeng/AnyCoder)** — a practical terminal AI coding agent built on the same architecture as CodingAgent but with litellm, session persistence, and 100+ model support. Use this one if you want a tool; use CodingAgent if you want to read source.
